...
import { roleEdit, setPerm, getPerm } from '@/api/perms/role'
...
const handleSubmit = async () => {
  ...
  formData.permIds = getDeptAllCheckedKeys()!
  formData.roleId = formData.id
  await setPerm(formData)
...
const setFormData = async (data: Record<any, any>) => {
  ...
  formData.permIds = await getPerm(formData.id)
  console.log('formData.permIds', formData.permIds)
}
